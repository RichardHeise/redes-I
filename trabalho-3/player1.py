...
while(True):
    if baton == True:
        ...
        data = network.create_package("player1", "player1", bet_value, bet, False, chips)
        ...
        if data["punter"] == "player1":
            data = network.create_package("player1", "player1", data["bet_value"], data["bet"], False, chips)
            network.send_msg(sock, data, receiver)
            ...
            if game.rollDices() == data["bet"]:
                ...
                data = network.create_package("player1", "player1", data["bet_value"], data["bet"], True, chips)
                network.send_msg(sock, data, receiver)
            ...
            data = network.create_package("player1", data["punter"], data["bet_value"], data["bet"], False, chips)
            network.send_msg(sock, data, receiver)
        ...
        if data["origin"] != "player1":
            ...
            if cover == "yes":
                ...
                data = network.create_package(data["origin"], "player1", int(data["bet_value"])+1, data["bet"], data["won"], data["chips"])
                network.send_msg(sock, data, receiver)
            ...
            if data["punter"] == "player1":
                ...
                if game.rollDices() == data["bet"]:
                    ...
                    chips = chips + game.points(data["bet"])
                    print("You now have " +str(chips)+ " chips ;)")
                    data = network.create_package(data["origin"], "player1", data["bet_value"], data["bet"], True, chips)
                    network.send_msg(sock, data, receiver)
                    ...
                    if chips <= 0:
                        print("You have no chips left, that means you lost the game :(")
                        data = network.create_package(data["origin"], "player1", data["bet_value"], data["bet"], False, chips)
                        network.send_msg(sock, data, receiver)
                        exit(0)
                    print("You now have " +str(chips)+ " chips ;)")
                    data = network.create_package(data["origin"], "player1", data["bet_value"], data["bet"], False, chips)
